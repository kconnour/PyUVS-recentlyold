...
from pyuvs.constants import apoapse_muv_day_night_voltage_boundary, apoapse_muv_failsafe_voltage
...
def determine_apoapse_muv_failsafe_files(hduls: hdulist) -> np.ndarray:
    ...
    return np.array([np.isclose(f['observation'].data['mcp_volt'][0], apoapse_muv_failsafe_voltage) for f in hduls])

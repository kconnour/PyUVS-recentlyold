...
import images.graphics as graphics
import pyuvs as pu
...
def make_histogram_equalized_variations_detector_image(orbit: int) -> None:
    ...
    try:
        #rgb_image = graphics.histogram_equalize_detector_image_variations(brightness, mask=mask) / 255
        rgb_image = graphics.histogram_equalize_detector_image_variations(brightness) / 255
    ...
    filename = f'{orbit_code}-heq_variations.png'
...
if __name__ == '__main__':
    ...
    save_location = Path('/home/kyle/iuvs/images/')

    n_cpus = mp.cpu_count()
    pool = mp.Pool(n_cpus -1)

    for orb in range(1, 1000):
        pool.apply_async(func=make_histogram_equalized_variations_detector_image, args=(orb,))
        #make_histogram_equalized_variations_detector_image(orb)

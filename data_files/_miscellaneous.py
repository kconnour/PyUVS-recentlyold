...
from constants import failsafe_voltage, day_night_voltage_boundary
...
def get_dayside_file_indices(file: h5py.File, integration_path: str, channel_integration_path: str) -> np.array:
    data_file = file[f'{integration_path}/data_file'][:]
    voltage = file[f'{channel_integration_path}/voltage'][:]
    return np.unique(data_file[voltage < day_night_voltage_boundary])


def get_failsafe_file_indices(file: h5py.File, integration_path: str, channel_integration_path: str) -> np.array:
    data_file = file[f'{integration_path}/data_file'][:]
    voltage = file[f'{channel_integration_path}/voltage'][:]
    return np.unique(data_file[voltage == failsafe_voltage])


def get_opportunity_file_indices(file: h5py.File, integration_path: str) -> np.array:
